<template>
  <div>
    ...
    <div class="dimen">
      ...
      <VantPicker Andlabel="所属渠道维度" DefauVal="北京海淀一" :AndList="['邮政','银行','银保合计','北京朝阳一','北京东城一','北京海淀一','其他']">
      </VantPicker>
    </div>
    <van-tabs v-model="active">
      <van-tab title="当日" name="0">
        <Day :DimeConfig="DimeConfig"></Day>
      </van-tab>
      <van-tab title="月度" name="1">
        <monthly :DimeConfig="DimeConfig"></monthly>
      </van-tab>
      <van-tab title="季度" name="2">
        <quarter :DimeConfig="DimeConfig"></quarter>
      </van-tab>
      <van-tab title="年度" name="3">
        <annual :DimeConfig="DimeConfig"></annual>
      </van-tab>
    </van-tabs>
  </div>
</template>
